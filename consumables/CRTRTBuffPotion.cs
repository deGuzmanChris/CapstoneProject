...
public class healingPotion : items
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.B))
        {
            ...
            //Gets CRTRT value into an int
            int currentCRTRT = statsGetter.GetTotalCRTRT();
            //Adds buff amount to current CRTRT
            int buffedCRTRT = currentCRTRT + buffAmount;
            float spellStart = Time.time;

            //Sets player CRTRT to new buffed amount
            ...
            if (playerObj != null)
            {
                ...
                if (player != null)
                {
                    player.SetTotalCRTRT(buffedCRTRT);
                }
            }
            ...
            while (Time.time < spellStart + buffDuration)
            {
                ...
                if (Time.time >= spellStart + buffDuration)
                {
                    if (playerObj != null)
                    {
                        ...
                        if (player != null)
                        {
                            player.SetTotalCRTRT(currentCRTRT);
                        }
                    }
                    break;
                }
